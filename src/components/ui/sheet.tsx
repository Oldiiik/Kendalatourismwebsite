...
const SheetClose = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(({ className, ...props }, ref) => {
  const context = React.useContext(SheetContext)
  return (
    <button
      ref={ref}
...
const sheetVariants = ({ side = "right", className = "" }: { side?: "top" | "bottom" | "left" | "right", className?: string } = {}) => {
  ...
  return cn(base, sides[side], className);
}
...
const SheetContent = React.forwardRef<HTMLDivElement, SheetContentProps>(({ side = "right", className, children, ...props }, ref) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex pointer-events-none">
       ...
       <div
        ref={ref}
        className={cn(sheetVariants({ side, className }), "pointer-events-auto")}
        {...props}
      >
        {children}
